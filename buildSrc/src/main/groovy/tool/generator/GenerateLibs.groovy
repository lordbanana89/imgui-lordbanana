...
class GenerateLibs extends DefaultTask {
    ...
    void generate() {
        ...
        def buildConfig = new BuildConfig('imgui-lordbanana-java', tmpDir, libsDirName, jniDir)
        ...
        if (forWindows)
            checkLibExist("windows64/imgui-lordbanana-java64.dll")
        if (forLinux)
            checkLibExist("linux64/libimgui-lordbanana-java64.so")
        if (forMac)
            checkLibExist("macosx64/libimgui-lordbanana-java64.dylib")
        if (forMacArm64)
            checkLibExist("macosxarm64/libimgui-lordbanana-java64.dylib")
    }
    ...
    BuildTarget createMacTarget(Architecture arch) {
        ...
        macTarget.libName = "libimgui-lordbanana-java64.dylib" // Lib for arm64 will be named the same for consistency.
