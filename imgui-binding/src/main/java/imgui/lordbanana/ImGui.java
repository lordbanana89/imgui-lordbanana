package imgui.lordbanana;

import imgui.lordbanana.assertion.ImAssertCallback;
import imgui.lordbanana.binding.annotation.ArgValue;
import imgui.lordbanana.binding.annotation.ArgVariant;
...
import imgui.lordbanana.callback.ImGuiInputTextCallback;
import imgui.lordbanana.internal.ImGuiContext;
import imgui.lordbanana.type.ImBoolean;
import imgui.lordbanana.type.ImDouble;
import imgui.lordbanana.type.ImFloat;
import imgui.lordbanana.type.ImInt;
import imgui.lordbanana.type.ImLong;
import imgui.lordbanana.type.ImShort;
import imgui.lordbanana.type.ImString;
...
public class ImGui {
    ...
    private static final String LIB_NAME_DEFAULT = "imgui-lordbanana-java64";
    private static final String LIB_TMP_DIR_PREFIX = "imgui-lordbanana-java-natives";
    ...
    private static final String NATIVE_RESOURCE_ROOT = "com/lordbanana/imgui-natives/";

    /**
     * Maps the host OS + CPU arch to the natives sub-folder produced by our CI. macOS is a
     * universal (x86_64 + arm64) binary so it carries no arch suffix. This arch-aware resolution
     * (which upstream SpaiR / the moulberry fork lack -- they branch on OS only) lets
     * linux-x86_64 and linux-arm64 natives coexist in a single multiplatform jar.
     */
    private static String nativeClassifierDir() {
        ...
        if (os.contains("win")) {
            ...
            return arm64 ? "linux-arm64" : "linux-x86_64";
        }
    }

    /** Opens the bundled native binary: arch-aware path first, flat path as a compatibility fallback. */
    private static InputStream openNativeResource(final String fullLibName) {
        final ClassLoader cl = ImGui.class.getClassLoader();
        InputStream is = cl.getResourceAsStream(NATIVE_RESOURCE_ROOT + nativeClassifierDir() + "/" + fullLibName);
        if (is == null) {
            is = cl.getResourceAsStream(NATIVE_RESOURCE_ROOT + fullLibName);
        }
        return is;
    }
    ...
    private static String tryLoadFromClasspath(final String fullLibName) {
        try (InputStream is = openNativeResource(fullLibName)) {
            if (is == null) {
                return null;
            }
    ...
    private static Optional<String> getVersionString() {
        ...
        try (InputStream is = ImGui.class.getResourceAsStream("/imgui/lordbanana/imgui-java.properties")) {
            if (is != null) {
                properties.load(is);
    ...
    */
    ...
    private static native void nInitInputTextData(); /*
        jclass jInputDataClass = env->FindClass("imgui/lordbanana/type/ImString$InputData");
        ...
        jclass jImString = env->FindClass("imgui/lordbanana/type/ImString");
        jImStringResizeInternalMID = env->GetMethodID(jImString, "resizeInternal", "(I)[B");

        jclass jCallback = env->FindClass("imgui/lordbanana/callback/ImGuiInputTextCallback");
